...
async def to_code(config: dict[str, Any]) -> None:
    ...
    if CONF_CUSTOM_HA_DISCOVERY_FILE in config:
        ...
        required = ("custom_ha_discovery_data", "custom_ha_discovery_chunks",
                    "CUSTOM_HA_DISCOVERY_CHUNK_COUNT", "CUSTOM_HA_DISCOVERY_MAX_CHUNK", "CUSTOM_HA_DISCOVERY_DATA_HASH")
        if not all(token in contents for token in required):
            raise cv.Invalid("custom_ha_discovery_file must be generated by custom_json/generate_custom_profile.py")
