...
typedef struct {
  ...
  // present in this sorted array. Used when appliance_api_parsing is enabled.
  const tiny_erd_t* valid_erds_filter;
  size_t valid_erds_filter_count;
  // Optional sorted array of string-type ERDs: when an ERD is in this array,
  // update_erd publishes the raw bytes as a null-terminated ASCII string
  // instead of hex.
  const tiny_erd_t* string_erds_filter;
  size_t string_erds_filter_count;
